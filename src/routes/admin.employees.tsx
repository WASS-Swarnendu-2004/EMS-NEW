...
const blank: CreateEmployeePayload = {
  ...
  // PF is enabled by default for new employees
  pfApplicable: true,
};
...
function Page() {
  ...
  const [form, setForm] =
    useState<CreateEmployeePayload>(
      blank,
    );

  const [fields, setFields] =
    useState<EmployeeField[]>(
      defaultEmployeeFields,
    );
  ...
  /*
   * We keep "roles" internally because
   * the existing designation API still
   * returns roles.
   */
  ...
  /*
   * Keep roleName because the existing
   * API expects roleName.
   */
  ...
  async function openEdit(
    ...
    const standardForm = {
      ...
      // IMPORTANT
      // Load PF value from backend
      pfApplicable:
        e.pfApplicable ?? false,
    };
    ...
    // -----------------------------------------
    // Create EmployeeField objects
    // for saved custom fields
    // -----------------------------------------
    ...
    // -----------------------------------------
    // Add saved custom fields without
    // duplicating them
    // -----------------------------------------
  ...
  function addField() {
    ...
    // Prevent custom field from
    // using the reserved PF name
    if (
      fieldName ===
      "pfApplicable"
    ) {
      toast.warning(
        "PF Applicable is a system field and cannot be added as a custom field",
      );
      return;
    }
    ...
    setFields((prev) => [
      ...prev,
      {
        ...
        name: fieldName,
        type:
          newField.type as EmployeeField["type"],
        removable: true,
      },
    ]);
  ...
  function removeField(
    ...
    setForm(
      updated as CreateEmployeePayload,
    );
  }
